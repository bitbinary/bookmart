...
export function BooksContext({ children }) {
    ...
    return (
        <Books.Provider value={booksContext}>{children}</Books.Provider>
    );
}
...
function SetBooksValues() {
    ...
    const updateFilters = (filter) => {
        ...
        const newFilter = {}
        Object.entries({ ...allBooksFilters, ...filter }).map(([filterKey, value], index) => {
            if (value !== null) {
    ...
    return {
        isAllBooksLoading, setIsAllBooksLoading,
        allBooksPageNumber, setAllBooksPageNumber, setBookRef,
        allBooksTotal, setAllBooksTotal,
        allBooksPerPage, setAllBooksPerPage,
        updateFilters, allBooksFilters,
        topSellers,
        setTopSellers,
        recentlyViewed,
        setRecentlyViewed,
